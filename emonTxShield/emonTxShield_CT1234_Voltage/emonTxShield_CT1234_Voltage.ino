...
void setup() 
{
 ...
 Serial.print(" Network: "); 
  ...
  if (CT1) ct1.current(1, 30);                                     // Setup emonTX CT channel (ADC input, calibration)
  if (CT2) ct2.current(2, 30);                                     // Calibration factor = CT ratio / burden resistance
  if (CT3) ct3.current(3, 30);                                     // emonTx Shield Calibration factor = (2340A / 0.05A) / 33 Ohms
  if (CT4) ct4.current(4, 30); 
  
  if (CT1) ct1.voltage(0, 225.938, 1.28);                                // ct.voltageTX(ADC input, calibration, phase_shift) - make sure to select correct calibration for AC-AC adapter  http://openenergymonitor.org/emon/modules/emontx/firmware/calibration                                         
  if (CT2) ct2.voltage(0, 224.92, 1);                                
  if (CT3) ct3.voltage(0, 224.92, 1);
  if (CT4) ct4.voltage(0, 224.92, 1);
