...
const int nodeID = 25;                                                  // emonTx RFM12B node ID
...
/*
typedef struct { int power1, power2, power3, power4; double Vrms;} PayloadTX;      // create structure - a neat way of packaging data for RF comms
*/
typedef struct {
  int power1;
  int power2;
...
void setup()
{
 ...
 Serial.print(" Network: ");
  ...
  if (CT1) ct1.current(1, 30.3);                                     // Setup emonTX CT channel (ADC input, calibration)
  if (CT2) ct2.current(2, 30.3);                                     // Calibration factor = CT ratio / burden resistance
  if (CT3) ct3.current(3, 30.3);                                     // emonTx Shield Calibration factor = (2340A / 0.05A) / 33 Ohms
  if (CT4) ct4.current(4, 30.3);

  if (CT1) ct1.voltage(0, 209.21, 2.3);                                // ct.voltageTX(ADC input, calibration, phase_shift) - make sure to select correct calibration for AC-AC adapter  http://openenergymonitor.org/emon/modules/emontx/firmware/calibration
  if (CT2) ct2.voltage(0, 209.21, 2.3);
  if (CT3) ct3.voltage(0, 209.21, 2.3);
  if (CT4) ct4.voltage(0, 209.21, 2.3);
